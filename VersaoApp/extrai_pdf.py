...
def extrai_arq():
    ...
    cursor.execute("CREATE TABLE IF NOT EXISTS Produtos(id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, data TEXT NOT NULL, produto TEXT NOT NULL, tipo TEXT, unidade_embalagem TEXT, valor_unidade_de_medidas REAL, unidade_de_medidas TEXT, situacao_mercado TEXT, valor_min NUMERIC, valor_m_c_do_dia NUMERIC, valor_max NUMERIC, valor_m_c_do_dia_anterior NUMERIC, valor_variacao REAL, estados_siglas TEXT, cidade TEXT)")
    ...
    # Construir o caminho relativo para a pasta 'baixados'
    caminho_da_pasta = os.path.join(diretorio_atual, 'baixados')
                ...
                for pag_num, df in enumerate(data, start=2):
                    ...
                    if len(selected_data) >= 1 :
                        for i in selected_data:
                            ...
                            print('Original: ', i )
                            print('Find', find_unidade)
                            #identifica as datas 
                            regex_datas = re.findall(r'(?i)\s\d{1,2}\sde\s(?:janeiro|fevereiro|março|abril|maio|junho|julho|agosto|setembro|outubro|novembro|dezembro)\sde\s\d{4}\b', find_unidade, re.IGNORECASE)
                            separar_data = str(regex_datas).split()
                            #definição do ano mes e dia
                            if regex_datas:
                                #.zfill(2) para que coloque 0 na frente dos números menores que 10 se não tiver
                                dia = str(separar_data[1]).zfill(2)
                                mes = str(separar_data[3]).zfill(2)
                                ano = separar_data[5].replace("']", "")
                                if mes in meses:
                                    mes = meses[mes]
                                data_numerica1= f'{ano}-{mes}-{dia}'
                            ...
                            regex_valor_unidade_medidas = re.findall(r'(\d+[.,]?\d*)\s*(?=kg|g|un|dz|gr|gs)', find_unidade, re.IGNORECASE)
